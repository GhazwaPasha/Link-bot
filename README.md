...
Public web-form submissions are the one exception: since the dashboard already holds the bot token read-only (to list channels/roles), it uses that same token to POST the review/output message straight to Discord's REST API itself (`apps/web/src/lib/discordDelivery.ts`), so the submitter's message normally lands instantly with no polling delay. The submission (and any uploaded images, in `submission_files`) is saved to the DB *first*, though — if that immediate post fails (429, timeout, Discord outage), the row stays `PENDING` and the bot's delivery poller (`apps/bot/src/deliveryPoller.ts`) retries it with backoff through discord.js's rate-limit-aware queue. A message posted either way is functionally identical, including fully working buttons: Approve/Reject clicks still route through Discord to the bot process (`apps/bot/src/events/interactionCreate.ts`), regardless of which process originally posted the message.
...
- **Bot → Render**: new Background Worker, root directory `apps/bot`, build command `pnpm install --frozen-lockfile && pnpm run build`, start command `pnpm start`. Add the same env vars.
  ...
  - **Bot → Oracle Cloud VM (alternative)**: this is what's actually running in production as of this writing — a systemd service on an Oracle "Always Free" VM instead of Render. Notably, the VM is too memory-constrained to build the project itself, so deploys ship a locally-built `dist/` rather than running `pnpm install`/`tsc` on the box. `docs/oracle-deployment.md` has the full deploy procedure — it's gitignored (contains the VM's real IP), so it's local-only and won't be there after a fresh clone.
...
- Web-form image uploads are capped at 5MB and PNG/JPEG/WebP/GIF only (`apps/web/src/app/api/public/forms/[formId]/submit/route.ts`). They're only staged in the DB until the submission's message is posted (then deleted) — if a submission with an image is later approved, the bot re-downloads it from the review message's own Discord attachment (Discord is already hosting it) before forwarding it to the output channel.
- If a web submission can't be delivered to Discord, it's never lost: transient failures are retried automatically by the bot (up to 8 attempts with backoff), and permanent ones (deleted channel, missing permissions) show up on the form's dashboard page with a Retry button once fixed.
